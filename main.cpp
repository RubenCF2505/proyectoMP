...
const int MAX_RESULTADOS_BUSQUEDA = 10;

using namespace std;

int main() {
    ListaPeliculas misPeliculas("peliculas.txt");
    misPeliculas.listado();

    Pelicula ** resultados = new Pelicula * [MAX_RESULTADOS_BUSQUEDA];
    int numResultados;

    cout << "Resultados de la búsqueda: " << endl;

    misPeliculas.buscarPeliculas("padrino", -1, -1, resultados, MAX_RESULTADOS_BUSQUEDA, numResultados);
    for (int i = 0; i < numResultados; i++) {
        cout << resultados[i]->toString() << endl;
    }

    cout << "Resultados de la búsqueda: " << endl;

    misPeliculas.buscarPeliculas("padrino", 1975, -1, resultados, MAX_RESULTADOS_BUSQUEDA, numResultados);
    for (int i = 0; i < numResultados; i++) {
        cout << resultados[i]->toString() << endl;
    }
    ...
    misPeliculas.buscarPeliculas("anillos", 2002, 2003, resultados, MAX_RESULTADOS_BUSQUEDA, numResultados);
    for (int i = 0; i < numResultados; i++) {
       cout << resultados[i]->toString() << endl;
    }
...
//    ListaUsuarios misUsuarios("usuarios.txt", "criticas.txt", misPeliculas);
//    misUsuarios.listado();
//
//
//    cout << "Eliminamos la pelicula de código 12" << endl;
//
//    misPeliculas.eliminarPelicula(12, misUsuarios);
//    misPeliculas.listado();
//
//    cout << "Eliminamos varias películas para que nos sobre el último bloque" << endl;
//
//    misPeliculas.eliminarPelicula(14, misUsuarios);
//    misPeliculas.eliminarPelicula(2, misUsuarios);
//    misPeliculas.eliminarPelicula(22, misUsuarios);
//    misPeliculas.eliminarPelicula(10, misUsuarios);
//    misPeliculas.listado();
//
